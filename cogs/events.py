...
class Events(commands.Cog):
    ...
    async def on_ready(self):
        ...
        await commands.Bot.get_channel(self.client, 1077307732757057656).send(f"Запуск завершен успешно! Время в которое включился бот [МСК]: `{time_str}` (ТЕСТОВЫЙ ЗАПУСК)")
